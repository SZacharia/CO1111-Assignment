...
function apiList() {                 //loads the list from the API
...
function handleList() {      //shows the list of the loaded data from the server
    console.log(this.responseText);
    let hunts = JSON.parse(this.responseText);
    let huntArray = hunts['treasureHunts'];
    let availableTH = '<ul>';
    for (let i = 0; i < huntArray.length; i++) {      //iterates to find all the available content
...
function LeaderboardStart(session){
    let url=API_LINK+'leaderboard?session='+getCookie("session")+'&sorted&limit='+20;
